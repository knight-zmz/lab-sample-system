...
-- 5.1 统计某类型样本数量
-- ----------------------------------------------------------------------------
CREATE FUNCTION fn_sample_count_by_type(p_type_id INT)
RETURNS INT
DETERMINISTIC
READS SQL DATA
BEGIN
...
-- 1. 触发器只做“辅助自动化 / 格式规范化”
-- 2. 不负责借还、移位、废弃等核心业务
-- 3. 不负责写 sample_transactions 历史流水
--
-- 这样做的原因很重要：
-- 旧版本的问题就在于“过程和触发器同时写业务记录”，导致重复记账。
-- ============================================================================
...
-- 用途：
-- 自动去掉首尾空格，并统一转为小写。
-- 这是一种“辅助性自动化”，不会影响业务主线，却能体现触发器能力。
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 用途：
-- 防止手工写入 samples 时出现未来采集日期。
-- 该校验与存储过程中的同类校验形成兜底关系。
-- ----------------------------------------------------------------------------
...
--
-- 本脚本提供以下过程：
-- 1. sp_register_sample  登记样本
-- 2. sp_borrow_sample    借用样本
-- 3. sp_return_sample    归还样本
-- 4. sp_move_sample      样本移位
-- 5. sp_dispose_sample   样本废弃
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 设计说明：
-- 1. 样本登记会同时生成当前状态记录与 CREATE 历史流水。
-- 2. sample_code 由过程统一生成，避免依赖触发器进行核心业务编号处理。
-- 3. 这里故意把“登记”也封装为过程，是为了让数据录入逻辑保持统一入口。
-- ----------------------------------------------------------------------------
...
--    - samples 当前状态
--    - borrow_records 借用单据
--    - sample_transactions 历史流水
-- 3. 使用 FOR UPDATE 锁定样本行，体现事务并发控制意识。
-- ----------------------------------------------------------------------------
...
-- 设计说明：
-- 1. 归还是借用流程的闭环。
-- 2. 必须找到“当前仍未结束”的借用记录，而不是随便更新某一条历史记录。
-- 3. 归还后样本重新变为 available。
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 设计说明：
-- 1. “移位”与“借出”是两个不同业务：移位时样本仍在库内。
-- 2. 这正是本系统用来展示“业务概念要分层”的一个关键点。
-- ----------------------------------------------------------------------------
...
-- 1. disposed 是终止性状态。
-- 2. 废弃后不允许再借用或移位。
-- 3. 为了避免语义混乱，这里要求样本必须先处于 available，不能在借出中直接废弃。
-- ----------------------------------------------------------------------------
