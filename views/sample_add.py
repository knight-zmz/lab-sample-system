...
def run():
    ...
    sample_name = st.text_input("样本名称", placeholder="例如：血清样本 A-01")
    ...
    with form_col1:
        ...
        project_name = st.selectbox("所属项目", list(project_options.keys()))
    with form_col2:
        created_by = st.selectbox("登记人", list(user_options.keys()))
        use_collected_date = st.checkbox("填写采集日期")
        collected_date = st.date_input("采集日期") if use_collected_date else None
        remark = st.text_area("登记备注", placeholder="例如：项目首批入库")
